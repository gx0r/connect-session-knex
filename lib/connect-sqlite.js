...
module.exports = function(connect) {
  ...
  SQLiteStore.prototype.get = function(sid, fn) {
    ...
    this.db.run('SELECT sess FROM ' + this.table + ' WHERE sid = ? AND ? <= expired;',
      [sid, now],
      function(err, rows) {
        if (err) fn(err);
  ...
  SQLiteStore.prototype.set = function(sid, sess, fn) {
    try {
      ...
      this.db.run('INSERT OR REPLACE INTO ' + this.table + ' VALUES (?, ?, ?);',
        [sid, expired, sess],
        function(err, rows) {
  ...
  SQLiteStore.prototype.clear = function(fn) {
    ...
    this.db.execute('DELETE FROM ' + this.table + ';', function(err) {
      if (err) fn(err);
      fn(null, true);
    });
  };
